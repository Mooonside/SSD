...
from ssd import ssd_arg_scope, ssd_vgg16, default_params, layers_anchors, layers_loss, layers_encoding
...
with arg_scope([get_variable], device=store_device):
    with tf.device('/GPU:0'):
        ...
        pos_loss = tf.reduce_mean(gather_pos_loss)
        neg_loss = tf.reduce_mean(gather_neg_loss)
        box_loss = tf.reduce_mean(gather_box_loss)
